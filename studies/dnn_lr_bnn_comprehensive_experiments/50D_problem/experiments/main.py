...
def bnn_single_run(seed: int) -> Dict:
    ...
    data = pickle.load(open("../data_generation/data_50D_example.pkl", "rb"))
    ...
    bnn_model = BNNWrapper(in_features=50,
...
def dnnlrbnn_single_run(seed: int) -> dict[str, Any]:
    ...
    responses = [hf_responses, lf_responses]
    ...
    mfdnnbnn = DNNLRBNN(design_space=design_space,
...
def dnn_bnn_single_run(seed: int) -> dict:
    ...
    mfbnn = DNNBNN(design_space=design_space,
                            lf_configure=lf_configure,
                            hf_configure=hf_configure)
...
def create_experiment_data() -> None:
    ...
    data.sample(sampler='random', n_samples=30, seed=1)
    ...
    data.store(filename='exp_{}'.format('50d_problem'))
...
def execute_experimentdata() -> None:
    ...
    data = f3dasm.ExperimentData.from_file(
        filename='exp_{}'.format('50d_problem'))
    # run the function
