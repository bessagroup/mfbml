...
def create_experiment_data() -> None:
    ...
    data.store(filename='exp_{}'.format('ablation_study_noisy'))
...
def run_method(
    ...
    mf_model = get_methods_for_noise_data(
        model_name='mf_rbf_gpr', design_space=func.input_domain)

    start_time = time.time()
    mf_model.train(sample_x, sample_y)
    ...
    start_time = time.time()
    sf_model.train(sample_x[0], sample_y[0])
    sf_pred_y, sf_pred_std = sf_model.predict(test_x[0], return_std=True)
    end_time = time.time()
...
def execute_experimentdata() -> None:
    ...
    data = f3dasm.ExperimentData.from_file(
        filename='exp_{}'.format('ablation_study_noisy'))
    # run the function
