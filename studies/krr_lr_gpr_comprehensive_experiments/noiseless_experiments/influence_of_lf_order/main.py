...
from mfbml.methods.krr_lr_gpr import \
    KernelRidgeLinearGaussianProcess as MFRBFKriging
...
def create_experiment_data() -> None:
    ...
    data.store(filename='exp_{}'.format('influence_of_lf_order'))
...
def run_method(
    ...
    sample_x = sampler.get_samples(
        num_samples=[num_hf*num_dim, num_lf*num_dim], seed=seed)
    # define noise samples
    sample_y = func(sample_x)

    # generate test samples
    test_x = sampler.get_samples(
        num_samples=[1000 * num_dim,1000 * num_dim], seed=seed+1)
    ...
    linear_without_bias_model.train(sample_x, sample_y)
    linear_without_bias_pred_y, _ = linear_without_bias_model.predict(
        X=test_x[0], return_std=True)
    end_time = time.time()
...
def execute_experimentdata() -> None:
    ...
    data = f3dasm.ExperimentData.from_file(
        filename='exp_{}'.format('influence_of_lf_order'))
    # run the function
