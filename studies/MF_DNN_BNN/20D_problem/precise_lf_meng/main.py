...
def main() -> None:
    ...
    # assemble the hf inputs and lf_corrected outputs
    hf_samples_scaled = torch.cat(
        (hf_samples_scaled, lf_responses_scaled), dim=1)
    ...
    # assemble the test samples
    test_samples_scaled = torch.cat(
        (test_samples_scaled, lf_responses_test_scaled), dim=1)

    # scale the sigma
    sigma_scaled = float(10.0 / torch.std(hf_responses))
    # define the bnn model
    bnn_model = BNNWrapper(in_features=21,
    ...
    df.to_csv("meng_mf_bnn_20D_problem_precise_lf.csv", index=False)
