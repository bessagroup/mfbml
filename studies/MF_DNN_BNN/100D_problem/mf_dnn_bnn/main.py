...
def main():
    ...
    # lf_responses_noiseless = data["lf_responses_noiseless"]
    ...
    hf_configure = {"in_features": 3,
                    "hidden_features": [300, 300],
                    "out_features": 1,
                    "activation": "Tanh",
                    "lr": 0.001,
                    "sigma": 0.05}
    ...
    # lf prediction
    lf_y = mfdnnbnn.lf_model.forward(test_samples)
    ...
    lf_nmae = normalized_mae(lf_responses_noisy.numpy(), lf_y.detach().numpy())
    lf_nrmse = normalized_rmse(
        lf_responses_noisy.numpy(), lf_y.detach().numpy())
    lf_r2 = r2_score(lf_responses_noisy.numpy(), lf_y.detach().numpy())
