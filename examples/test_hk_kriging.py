...
# update sample_x['hf']
sample_x['hf'] = np.array([0.0, 0.4, 0.6, 0.8, 1.0]).reshape((-1, 1))
...
model = HierarchicalKriging(design_space=func.input_domain)
model.train(samples=sample_x, responses=sample_y)
pred_y, pred_std = model.predict(test_x=test_x, return_std=True)
