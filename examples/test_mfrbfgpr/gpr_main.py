...
mfrbfgp = GaussianProcess(
    design_space=func.input_domain, noise_prior=None, optimizer_restart=20
)
mfrbfgp.train(sample_x=sample_x["hf"], sample_y=sample_y["hf"])
...
ax.set_ylabel("noise")
fig.colorbar(cs)
plt.savefig("log_likelihood.png", dpi=300, bbox_inches="tight")
plt.show()
...
ax.plot(test_x, mfrbfgp_pred_y, label="gp")
ax.fill_between(
    test_x.flatten(),
...
# trying to plot the landscape of log_likelihood function
